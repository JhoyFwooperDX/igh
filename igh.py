...
\t[white]╚══╗║║║║║║║║║╔══╣║─╔╣╔══╝║╔╗╔╣╔══╣╔═╗║║─║║╔╗╔╣║╚╗║║
\t[white]║╚═╝╠╣╠╣║║║║║║──║╚═╝║╚══╗║║║╚╣╚══╣╚═╝║╚═╝║║║╚╣║─║║║
\t[white]╚═══╩══╩╝╚╝╚╩╝──╚═══╩═══╝╚╝╚═╩═══╩═══╩═══╩╝╚═╩╝─╚═╝Made By [red]Indonesia [white]Coder[/]
...
def cekAPI(cookie):
    ...
    try:
        ...
        cetak(nel(f'\tAkun tumbal anda terkena checkpoint', style='red', title='ðŸ˜­ðŸ˜­ðŸ˜­'))
        time.sleep(3)
        os.remove('.kukis.log')
        os.remove('.username')
        exit()
...
def login_kamu():
        try:
            ...
            banner()
            __anjing__ = '[white][[cyan]â€¢>[white]] [green]Disarankan Login Menggunakan [yellow]Cookies!!\n[white][[cyan]â€¢>[white]] [green]Sebab Kalau Lewat Manual Akun Gampang Terkena [yellow]Checkpoint!![white]'
            ...
            if loginpil=='1':
                wel = '[white][[cyan]â€¢[white]] Masukkan Username And Cookies Untuk Login'
                ...
                print(f'  {C}[{B}â€¢{C}]{H} Login SuccessFull.......!!{C}');time.sleep(2);os.system('python nano.py')
            elif loginpil == '2':
                login()
            elif loginpil == '3':
                os.system('xdg-open https://m.youtube.com');login_kamu()
            else:
                print(f'  â”œâ”€â”€> Yang bener lah kontol ');exit()
        ex,user=cekAPI(kuki)
        cookie={'cookie':kuki}
...
def login():
    global external
    try:
        cetak(nel(f'\t{crot} Gunakan username dan paasword untuk login {crot}', style='bold white', title='ðŸŒŸ'))
        us=input(f'{C}  â”œâ”€â”€> Masukkan username : ')
        pw=stdiomask.getpass(prompt=f'{C}  â”œâ”€â”€> Masukkan password : ')
    except KeyboardInterrupt:
        print(f'  â”œâ”€â”€> Yang bener lh kontoll {C}');exit()
    x=instagramAPI(us,pw).loginAPI()
    if x.get('status')=='success':
        open('.username','a').write(us)
        open('.kukis.log','a').write(x.get('cookie'))
        cookie={'cookie':x.get('cookie')}
        print(f'  â”œâ”€â”€> {H}Login Successfully {C}');exit()
    elif x.get('status')=='checkpoint':
        print(f'  â”œâ”€â”€> {K}Akun anda terkena checkpoint{C} ');time.sleep(2);login()
    else:
        print(f'  â”œâ”€â”€> {K}Username atau password salah{C} ');time.sleep(2);login()
...
[{H}â€¢{C}]Following      : {following}'''
    ...
    def passwordAPI(self,xnx):
        ...
        cetak(nel(komb1))
        c=input(f'{CY}[â€¢] Masukan Pilihan :{C} ')
        if c=='1':
    ...
    def menu(self):
        ...
        if c=='':
            ...
            for s in g:
                ...
                if xc=="checkpoint":
                    print(f"""
 [{M}+{C}] {M}ðŒð€ðŒðð”ð’ ð‚ð„ðŠððŽðˆð“{C}:
  {M}|{C}
